import SwadeBaseActorSheet from "../../systems/swade/module/sheets/SwadeBaseActorSheet.js";
...
class LootSwadeNPC extends SwadeBaseActorSheet {

    static SOCKET = "module.lootsheetswade";
    ...
    get template() {
        ...
        const path = "systems/swade/templates/actors/";
        if (!game.user.isGM && this.actor.limited) return path + "limited-sheet.html";
        return "modules/lootsheetswade/template/npc-sheet.html";
    }
    ...
    async getData() {
        ...
        let lootsheettype = await this.actor.getFlag("lootsheetswade", "lootsheettype");
        if (!lootsheettype) await this.actor.setFlag("lootsheetswade", "lootsheettype", "Loot");
        lootsheettype = await this.actor.getFlag("lootsheetswade", "lootsheettype");


        let priceModifier = 1.0;
        if (lootsheettype === "Merchant") {
            priceModifier = await this.actor.getFlag("lootsheetswade", "priceModifier");
            if (!priceModifier) await this.actor.setFlag("lootsheetswade", "priceModifier", 1.0);
            priceModifier = await this.actor.getFlag("lootsheetswade", "priceModifier");
        }
        ...
        sheetData.lootCurrency = game.settings.get("lootsheetswade", "lootCurrency");
        sheetData.lootAll = game.settings.get("lootsheetswade", "lootAll");
    ...
    async _merchantSettingChange(event, html) {
        ...
        const moduleNamespace = "lootsheetswade";
    ...
    async _changeSheetType(event, html) {
        ...
        await currentActor.setFlag("lootsheetswade", "lootsheettype", selectedItem);
    ...
    _buyItem(event, all = 0) {
        ...
        if (item.data.quantity === packet.quantity) {
            console.log("lootsheetswade", "Sending buy request to " + targetGm.name, packet);
            game.socket.emit(LootSheetSwade.SOCKET, packet);
            return;
        }
    ...
    _lootItem(event, all = 0) {
        ...
        if (targetItem.data.quantity === item.quantity) {
            console.log("lootsheetswade", "Sending loot request to " + targetGm.name, packet);
            game.socket.emit(LootSheetSwade.SOCKET, packet);
            return;
        }
    ...
    _lootCoins(event) {
        event.preventDefault();
        if (!game.settings.get("lootsheetswade", "lootCurrency")) {
            return;
        }
    ...
    async _priceModifier(event) {
        ...
        let priceModifier = await this.actor.getFlag("lootsheetswade", "priceModifier");
        ...
        let d = new Dialog({
            ...
            buttons: {
                one: {
                    icon: '<i class="fas fa-check"></i>',
                    label: "Update",
                    callback: () => this.actor.setFlag("lootsheetswade", "priceModifier", document.getElementById("price-modifier-percent").value / 100)
                },
                two: {
                    icon: '<i class="fas fa-times"></i>',
                    label: "Cancel",
    ...
    _distributeCoins(event) {
        ...
        console.log("lootsheetswade", "Sending distribute coins request to " + targetGm.name, packet);
        game.socket.emit(LootSheetSwade.SOCKET, packet);
    }
...
Hooks.on('preCreateOwnedItem', (actor, item, data) => {
    ...
    if ((actor.data.flags.core || {}).sheetClass === "swade.LootSheetSwade") {
        if (item.type === "spell") {
            //console.log("Loot Sheet | dragged spell item", item);

            let changeScrollIcon = game.settings.get("lootsheetswade", "changeScrollIcon");

            if (changeScrollIcon) item.img = "modules/lootsheetswade/icons/Scroll" + item.data.level + ".png";
...
Hooks.once("init", () => {
    ...
    game.settings.register("lootsheetswade", "convertCurrency", {
    ...
    game.settings.register("lootsheetswade", "changeScrollIcon", {
    ...
    game.settings.register("lootsheetswade", "buyChat", {
    ...
    game.settings.register("lootsheetswade", "lootCurrency", {
        name: "Loot currency?",
    ...
    game.settings.register("lootsheetswade", "lootAll", {
        name: "Loot all?",
    ...
    function chatMessage(speaker, owner, message, item) {
        if (game.settings.get("lootsheetswade", "buyChat")) {
            message = `
            <div class="swade chat-card item-card" data-actor-id="${owner._id}" data-item-id="${item._id}">
    ...
    function errorMessageToActor(target, message) {
        game.socket.emit(LootSheetSwade.SOCKET, {
            type: "error",
            targetId: target.id,
            message: message
        });
    ...
    async function transaction(seller, buyer, itemId, quantity) {
        ...
        let sellerModifier = seller.getFlag("lootsheetswade", "priceModifier");
        ...
        const conversionRate = { 
            "pp": CONFIG.SWADE.currencyConversion.gp.each,
            "gp": 1, 
            "ep": 1 / CONFIG.SWADE.currencyConversion.ep.each,
            "sp": 1 / CONFIG.SWADE.currencyConversion.ep.each / CONFIG.SWADE.currencyConversion.sp.each,
            "cp": 1 / CONFIG.SWADE.currencyConversion.ep.each / CONFIG.SWADE.currencyConversion.sp.each / CONFIG.SWADE.currencyConversion.cp.each
        };
        ...
        let convertCurrency = game.settings.get("lootsheetswade", "convertCurrency");
    ...
    game.socket.on(LootSheetSwade.SOCKET, data => {
